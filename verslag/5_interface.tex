...
We kunnen echter niet zomaar dioden op de signaallijnen plaatsen. Als we dit wel zouden doen, zou de het volledige spanningsverschil tussen de elektrische grond en de poortpin (wat $5$ volt bedraagt, dankzij de interne pull-up) over de diode geplaatst worden, waardoor die beschadigd zou worden. Om de diodes te beschermen tegen deze schadelijke voorwaartse spanningsval, zullen we bij elk van de drie poortpinnen een extra weerstand plaatsen. Hierdoor zal de overmatige spanning over deze weerstand komen te staan, waardoor de diodes slechts het spanningsverschil zullen moeten verwerken waarvoor ze gemaakt zijn ($0.5$ volt, zoals hierboven bepaald). De keuze van deze weerstand is echter niet zo eenvoudig als het op het eerste zicht zou lijken: we moeten immers zorgen dat het spanningsniveau aan het hoogimpedant meetcircuit binnen de microcontroller zich in alle gevallen binnen het gebied bevindt dat overeen komt met een laag signaal.

De datasheet van de microcontroller stelt dat een laag signaal gedetecteerd wordt tussen $0$ volt en $0.6$ keer de bronspanning. Aangezien de microcontroller zich rechtstreeks voedt met de USB bronspanning, dewelke $5$ volt bedraagt, moet de spanning aan het meetcircuit vallen tussen $0$ en $3$ volt. Indien we deze waarden combineren met de worst-case waarde voor de interne pull-up weerstand, zijnde $20$ kilo-ohm (dit aangezien de interne pull-up zich samen met onze beschermingsweerstand gedraagt zoals een spanningsdeler, waardoor een lagere waarde voor de pull-up weerstand resulteert in een hogere spanning over de beschermingsweerstand en dus aan het meetcircuit), kunnen we de mogelijke waarden voor onze beschermingsweerstand perfect berekenen. We behandelen hierbij opnieuw het worst-case geval waarbij zich het maximale aantal dioden bevindt tussen de elektrische grond en de beschermingsweerstand, omdat die steeds een inbreng hebben van een extra $0.5$ volt die niet te reduceren valt door de beschermingsweerstand te veranderen.
\begin{equation}
\begin{split}
V_{cc} &= 5 V \\
R_{pullup} &= 20 kΩ \\
V_{dioden} &= 3 . 0.5 V \\
           &= 1.5 V \\ 
V_{spanningsdeler} &= V_{cc} - V_{dioden} \\
                   &= 3.5 V
\end{split}
\end{equation}
\begin{equation}
\begin{split}
V_{bescherming} &= \frac{V_{spanningsdeler}}{R_{pullup} + R_{bescherming}} . R_{bescherming} \\
                &= \frac{3.5 V}{20 kΩ + R_{bescherming}} . R_{bescherming} \\
\end{split}
\end{equation}
\begin{equation}
\begin{split}
V_{bescherming} + V_{dioden} &< 3 V \\
\frac{3.5 V}{20 kΩ + R_{bescherming}} . R_{bescherming} + 1.5 V &< 3 V \\
\frac{3.5 V}{20 kΩ + R_{bescherming}} . R_{bescherming} &< 1.5 V \\
R_{bescherming} &< 15 kΩ \\
\end{split}
\end{equation}
